module OpenAi::BlogPosts
  class GenerateVideoArticle < OpenAi::GenerateStream
    include Interactor
    GENERATION_USED = 6

    private
    def prompt
      "You are an expert in digital content creation.From this submitted video transcription #{transcribe}, write an article with consistent headings, subheadings and paragraphs using [#{params[:tone]}] tone of voice with length #{params[:length]} characters and follow this detailed instructions:
      ...
      #{related_products_prompt}
      #{related_blogs_prompt}
    ...
    def related_products_prompt
      return "" unless params[:related_products].present?

      "Include these products in the article: #{params[:related_products]}"
    end

    def related_blogs_prompt
      return "" unless params[:related_blogs].present?

      "Include these blogs in the article: #{params[:related_blogs]}"
    end
    ...
    def stream_channel
      "open_ai_stream_article_#{shop.shopify_domain}"
    end
