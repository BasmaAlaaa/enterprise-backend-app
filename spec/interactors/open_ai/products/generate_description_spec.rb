...
RSpec.describe OpenAi::Products::GenerateDescription, type: :interactor do
  describe '#call' do
    ...
    let(:mock_response){
      {
        'choices' => [
          { 'message' => { 'content' => '{"description": "Detailed product description for Summer casual green jacket.", "features": ["lightweight", "durable"]}' } },
          { 'message' => { 'content' => '{"description": "Another variant of the product description.", "features": ["stylish", "comfortable"]}' } },
          { 'message' => { 'content' => '{"description": "Yet another variant, more creative.", "features": ["colorful", "breathable"]}' } }
        ],
    ...
    context 'succeeds' do
      ...
      it 'updates statistics and subscription correctly' do
        ...
        statistics.reload
        subscription.reload
        expect(statistics.total_words).to eq(2) and expect(statistics.product_title).to eq(0) and expect(statistics.product_description).to eq(1) and expect(statistics.product_seo).to eq(0)
        expect(subscription.remaining_generations).to eq(998)
      end
    end
