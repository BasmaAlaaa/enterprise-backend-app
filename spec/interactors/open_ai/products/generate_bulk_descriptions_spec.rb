...
RSpec.describe OpenAi::Products::GenerateBulkDescriptions, type: :interactor do
  describe '#call' do
    ...
    let(:mock_response){
      {
        'choices' => [
          { 'message' => { 'content' => '{"id": 1, "description": "Description for green jacket."}' } },
          { 'message' => { 'content' => '{"id": 2, "description": "Description for floral white top."}' } },
          { 'message' => { 'content' => '{"id": 3, "description": "Description for black jeans."}' } },
          { 'message' => { 'content' => '{"id": 4, "description": "Description for red socks."}' } }
        ],
    ...
    context 'succeeds' do
      ...
      it 'updates statistics and subscription correctly' do
        ...
        statistics.reload
        subscription.reload
        expect(statistics.total_words).to eq(8) and expect(statistics.product_title).to eq(0) and expect(statistics.product_description).to eq(4) and expect(statistics.product_seo).to eq(0)
        expect(subscription.remaining_generations).to eq(992)
      end
    end
