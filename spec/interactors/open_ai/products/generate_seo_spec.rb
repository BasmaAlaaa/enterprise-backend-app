...
RSpec.describe OpenAi::Products::GenerateSeo, type: :interactor do
  describe '#call' do
    ...
    let(:params) {
      {
        title: "green jacket",
        language: "Arabic",
        product_description: "Get ready to embrace the summer heat with our new line of casual cotton clothing!"
      }
    }
    let(:mock_response){
      {
        'choices' => [
          { 'message' => { 'content' => JSON.generate({'seo_description' => "Optimized SEO Description", 'seo_title' => "Optimized SEO Title"}) } }
        ],
    ...
    context 'succeeds' do
      ...
      it 'updates statistics and subscription correctly' do
        ...
        statistics.reload
        subscription.reload
        expect(statistics.total_words).to eq(1) and expect(statistics.product_title).to eq(0) and expect(statistics.product_description).to eq(0) and expect(statistics.product_seo).to eq(1)
